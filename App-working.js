...
const DADOS_EXEMPLO = [
  {
    id: 1,
    descricao: 'Corte área central',
    localizacao: 'Setor A - Entrada principal',
    area: '500m²',
  ...
  {
    id: 2,
    descricao: 'Limpeza terreno lateral',
    localizacao: 'Setor B - Lateral direita',
    area: '300m²',
  ...
  {
    id: 3,
    descricao: 'Manutenção cerca perimetral',
    localizacao: 'Setor C - Fundos',
    area: '150m²',
    dataNecessidade: '20/07/2025',
    status: 'pendente',
    dataInicio: null,
    dataConclusao: null,
  },
...
• ${item.descricao}
  ...
  🏷️ Status: ${getStatusText(item.status)}
  ${item.dataInicio ? `▶️ Iniciado: ${new Date(item.dataInicio).toLocaleDateString('pt-BR')}` : ''}
  ${item.dataConclusao ? `✅ Concluído: ${new Date(item.dataConclusao).toLocaleDateString('pt-BR')}` : ''}
`).join('')}
